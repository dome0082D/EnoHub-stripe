...
mongoose.connect(process.env.MONGO_URI)
    .then(() => console.log('✅ DATABASE: EnoHub Diamond V2 Connesso'))
    .catch(e => console.log('❌ DATABASE ERROR:', e.message));

// SCHEMA UTENTE POTENZIATO
const User = mongoose.model('User', new mongoose.Schema({
    ...
    // Contenuti Premium (visibili solo se sbloccati)
    ...
    unlockedContacts: [String], following: [String], // Logica social
    unlocksRemaining: { type: Number, default: 0 },
    isVerified: { type: Boolean, default: false } // Spunta Blu
}));
...
io.on('connection', (socket) => {
    ...
    socket.on('send_msg', async (d) => {
        ...
        io.to(d.to).emit('notification', { title: d.fromName, body: d.text || "📎 File" });
    });
...
server.listen(PORT, () => console.log(`🚀 EnoHub 2.0 Diamond Active on ${PORT}`));